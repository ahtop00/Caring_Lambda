...
class IngestProcessor:
    ...
    def run_for_fetcher(self, fetcher: BaseWelfareFetcher, event_params: dict):
        ...
        for page in range(self.start_page, end_page):
            ...
            # 2. Filter (DTO의 service_id 또는 job_id 사용)
            ...
            if not ids_from_api:
                # service_id가 없다면(JobOpeningDTO), 중복 검사를 스킵 (이미 repo가 처리함)
                pass
            ...
            # (중요) _save_and_publish가 실패(0 반환)하면 커밋하지 않음
            if page_inserted_count > 0:
                logger.info(f"페이지 {page} 작업 완료. 커밋을 준비합니다.")
                # (커밋은 모든 페이지가 끝난 후 한 번만 수행하는 것이 좋음)
                # [수정] 페이지 단위로 커밋/롤백
                try:
                    self.repo.commit()
    ...
    def _save_and_publish(self, data_list: List[Tuple]) -> int:
        ...
        try:
            ...
            if inserted_count > 0:
                ...
                logger.warning(f"DB 일괄 삽입 결과가 0건입니다.")
                return 0

        except Exception as e:
            logger.error(f"DB/SQS 처리 중 오류: {e}. (롤백은 상위에서 수행)", exc_info=True)
            return 0 # 0을 반환하여 상위 메소드가 롤백하도록 함
